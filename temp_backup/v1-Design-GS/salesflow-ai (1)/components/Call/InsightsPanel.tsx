                       ...
                       <div className="px-4 pb-4 pt-0 border-t border-slate-50 dark:border-slate-800/50">
                          ...
                          <div className="mt-3 space-y-3">
                             ...
                             {section.id === 'needs' && (
                               <>
                                 {[
                                   { text: 'אינטגרציה ל-ERP הקיים', status: 'full' },
                                   { text: 'גישה מהנייד לאנשי שטח', status: 'partial' },
                                   { text: 'דוחות מותאמים אישית', status: 'none' }
                                 ].map((item, i) => (
                                   <div key={i} className="flex items-start justify-between group">
                                      <span className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed">{item.text}</span>
                                      <div className={`w-2 h-2 rounded-full mt-1.5 flex-shrink-0 ${
                                        item.status === 'full' ? 'bg-emerald-500' : 
                                        item.status === 'partial' ? 'bg-amber-400' : 'bg-slate-300 dark:bg-slate-700'
                                      }`} title={item.status === 'full' ? 'כוסה מלא' : 'כוסה חלקית'}></div>
                                   </div>
                                 ))}
                               </>
                             )}
                             ...
                             {section.id === 'vision' && (
                               <>
                                 {[
                                   { text: 'חיסכון של 5 שעות שבועיות לנציג', status: 'full' },
                                   { text: 'שקיפות בזמן אמת להנהלה', status: 'full' }
                                 ].map((item, i) => (
                                   <div key={i} className="flex items-start justify-between group">
                                      <span className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed">{item.text}</span>
                                      <div className="w-2 h-2 rounded-full mt-1.5 flex-shrink-0 bg-emerald-500"></div>
                                   </div>
                                 ))}
                               </>
                             )}
                                 ...
                                 <div className="space-y-3">
                                   <div className="flex flex-col gap-1.5">
                                      <div className="flex justify-between items-start">
                                         <Badge variant="danger" className="text-[9px] px-1.5 py-0">גבוה</Badge>
                                      </div>
                                      <p className="text-xs text-slate-800 dark:text-slate-200 font-medium">הזנת נתונים ידנית ואיטית</p>
                                   </div>
                                   <div className="h-px bg-slate-100 dark:bg-slate-800"></div>
                                   <div className="flex flex-col gap-1.5">
                                      <div className="flex justify-between items-start">
                                         <Badge variant="warning" className="text-[9px] px-1.5 py-0">בינוני</Badge>
                                      </div>
                                      <p className="text-xs text-slate-800 dark:text-slate-200 font-medium">חוסר יכולת לעבוד באופליין</p>
                                   </div>
                                 </div>
                               </>
                             )}
                             ...
                             {section.id === 'objections' && (
                               <>
                                 <div className="space-y-3">
                                   {/* Objection 1 */}
                                   <div className="bg-rose-50 dark:bg-rose-900/10 p-2.5 rounded-lg border border-rose-100 dark:border-rose-900/30">
                                      <div className="flex justify-between items-start mb-1.5">
                                         <div className="flex items-center gap-1.5">
                                            <Flame className="w-3 h-3 text-rose-500 fill-rose-500" />
                                            <span className="text-[10px] font-bold text-rose-700 dark:text-rose-400">מחיר</span>
                                         </div>
                                         <span className="text-[9px] border border-rose-200 text-rose-600 px-1.5 rounded-sm bg-white dark:bg-slate-900">פתוחה</span>
                                      </div>
                                      <p className="text-xs text-slate-700 dark:text-slate-300 italic">"התקציב שלנו לרבעון מוגבל מאוד..."</p>
                                   </div>
                                   {/* Objection 2 */}
                                   <div className="bg-emerald-50 dark:bg-emerald-900/10 p-2.5 rounded-lg border border-emerald-100 dark:border-emerald-900/30 opacity-75">
                                      <div className="flex justify-between items-start mb-1.5">
                                         <div className="flex items-center gap-1.5">
                                            <CheckCircle2 className="w-3 h-3 text-emerald-500" />
                                            <span className="text-[10px] font-bold text-emerald-700 dark:text-emerald-400">תזמון</span>
                                         </div>
                                         <span className="text-[9px] border border-emerald-200 text-emerald-600 px-1.5 rounded-sm bg-white dark:bg-slate-900">טופלה</span>
                                      </div>
                                      <p className="text-xs text-slate-700 dark:text-slate-300 italic line-through">"זמן ההטמעה ארוך מדי"</p>
                                   </div>
                                 </div>
                               </>
                             )}

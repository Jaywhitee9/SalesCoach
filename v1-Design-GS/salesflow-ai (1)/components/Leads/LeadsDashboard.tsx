...
import { MOCK_LEADS, TEAM_MEMBERS } from '../../constants';
...
export const LeadsDashboard: React.FC<LeadsDashboardProps> = ({ isDarkMode }) => {
  ...
  const [tempSelectedStatuses, setTempSelectedStatuses] = useState<string[]>([]); // Buffer for "Apply"

  // Owner Filter (Single-select)
  ...
  // Sorting Helper for Hebrew Time Strings
  ...
  const handleSaveNewLead = (newLeadData: Partial<Lead>) => {
    console.log("Saving new lead:", newLeadData);
  };
             ...
             <div className="hidden md:flex items-center gap-2">
                ...
                <div className="relative">
                  <button 
                    ...
                    className={`flex items-center gap-2 px-3 py-2 border rounded-lg text-sm transition-colors ${
                      openFilter === 'owner' || selectedOwner !== 'all'
                  ...
                  >
                    ...
                    {selectedOwner === 'all' ? 'בעלים' : TEAM_MEMBERS.find(u => u.id === selectedOwner)?.name}
                ...
                <div className="relative">
                  <button 
                    ...
                    className={`flex items-center gap-2 px-3 py-2 border rounded-lg text-sm transition-colors ${
                      openFilter === 'status' || selectedStatuses.length > 0
                        ? 'bg-brand-50 border-brand-200 text-brand-600 dark:bg-brand-900/20 dark:border-brand-800 dark:text-brand-400' 
                        : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-300 hover:border-brand-300'
                    }`}
                  >
      ...
      {selectedIds.length > 0 && (
        <div className="absolute top-[140px] left-1/2 -translate-x-1/2 z-20 bg-slate-900 text-white px-6 py-3 rounded-xl shadow-lg flex items-center gap-6 animate-in slide-in-from-top-2 fade-in duration-200">
           ...
           <div className="flex items-center gap-2">
              ...
              <button className="flex items-center gap-2 text-sm text-rose-300 hover:text-rose-100 px-2 py-1 rounded hover:bg-rose-500/20 transition-colors">

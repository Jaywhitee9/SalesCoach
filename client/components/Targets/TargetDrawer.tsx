...
export const TargetDrawer: React.FC<TargetDrawerProps> = ({ targetData, user, onClose, onSave }) => {
   ...
   React.useEffect(() => {
      if (targetData) {
         setFormData({
            calls: targetData.calls.target,
            connectedCalls: targetData.connectedCalls.target,
            talkTime: targetData.talkTime.target,
            conversion: 15, // default
            quality: 80,    // default
            deals: targetData.deals.target * 20,
            revenue: targetData.revenue.target
         });
      }
   }, [targetData]);

   if (!targetData || !user || !formData) return null;
                     ...
                     <div>
                        ...
                        <div className="relative">
                           <input
                              ...
                              onChange={(e) => handleChange('connectedCalls', e.target.value)}
                              className="w-full px-3 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-sm outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent transition-all"
                           />
                           <span className="absolute left-3 top-1/2 -translate-y-1/2 text-xs text-slate-400 pointer-events-none">שיחות</span>
                        </div>
                     ...
                     <div className="col-span-2">
                        ...
                        <div className="relative">
                           <input
                              ...
                              onChange={(e) => handleChange('talkTime', e.target.value)}
                              className="w-full px-3 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-sm outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent transition-all"
                           />
                     ...
                     <div>
                        ...
                        <div className="relative">
                           <input
                              ...
                              onChange={(e) => handleChange('deals', e.target.value)}
                              className="w-full px-3 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-sm outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent transition-all"
                           />

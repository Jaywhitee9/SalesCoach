...
export const TargetsDashboard: React.FC<TargetsDashboardProps> = ({ isDarkMode, orgId }) => {
   ...
   const getUserTarget = (userId: string) => {
      ...
      // Return formatted RepTargets object or defaults
      return {
         userId,
         calls: { target: userTargets?.calls || 40, current: 0 }, // Current still mock/0 for now
         connectedCalls: { target: userTargets?.connectedCalls || 15, current: 0 },
         talkTime: { target: userTargets?.talkTime || 120, current: 0 },
         deals: { target: (userTargets?.deals || 5) / 20.0, current: 0 }, // adjust back from form calculation if needed
